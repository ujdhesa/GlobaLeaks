...
def db_create_user(session, tid, request, language):
    ...
    db_set_user_password(tid, user, request['password'])
    ...
    if password:
        ...
        if user.crypto_pub_key and user_session.ek:
            ...
            if user_session.user_tid == 1:
                cc = GCE.asymmetric_decrypt(crypto_escrow_prv_key, Base64Encoder.decode(user.crypto_escrow_bkp1_key))
            else:
                cc = GCE.asymmetric_decrypt(crypto_escrow_prv_key, Base64Encoder.decode(user.crypto_escrow_bkp2_key))

        if not user.crypto_pub_key or cc:
            db_set_user_password(tid, user, password, cc)

        State.log(tid=tid, type='change_password', user_id=user_session.id, object_id=user.id)
